/*
  # Fix Account Deletion and Signup Policies

  1. New Policies
    - Add INSERT policy for users table (only if it doesn't exist)
    
  2. New Functions
    - `delete_user_account()`: Internal function to delete user data and auth record
    - `delete_my_account()`: Public RPC function for users to delete their own account
...
-- Add INSERT policy for users table (only if it doesn't already exist)
DO $$
BEGIN
  IF NOT EXISTS (
